...
export default function NoticeDetailClient({ id, initialDetail }: Props) {
  ...
  const detailFromStore = useNoticeDetailStore((state) => state.noticeDetail);
  const detailNotice = detailFromStore ?? initialDetail;

  const fetchFileDownload = useNoticeDownloadFetch();

  const [isEditMode, setIsEditMode] = useState(false);
  const [newNotice, setNewNotice] = useState<NewNoticeState>({
    id: initialDetail?.id ?? null,
    title: initialDetail?.title ?? '',
    content: initialDetail?.cont ?? '',
  });
    ...
    selectedFiles.forEach(async (file, i) => {
      ...
      try {
        ...
        setFiles((prev) =>
          prev.map((p, j) =>
            j === idx
              ? { ...p, fileId: res.fileId, uploadStatus: 'done' }
              : p
          )
        );
      } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    if (files.some((f) => f.uploadStatus === 'failed')) {
      await alertDialog(
        '업로드 실패한 파일이 있습니다. 제거 후 다시 시도해주세요.',
        'error'
      );
      return;
    }
    ...
    type FileChange = {
      id: number;
      fileProcessStatus: 'CREATE' | 'DELETE' | 'NORMAL';
    };
    ...
    files
      ...
      .forEach((f) =>
        fileChanges.push({ id: f.fileId!, fileProcessStatus: 'CREATE' })
      );
    ...
    if (isEditMode) {
      attachedFiles
        ...
        .forEach((f) =>
          fileChanges.push({ id: f.id, fileProcessStatus: 'DELETE' })
        );
    }
    ...
    if (legacyDeletes.length > 0) {
      console.warn(
        '[notice] legacy 파일 삭제 요청은 현재 무시됨',
        legacyDeletes
      );
    }
  ...
  useEffect(() => {
    if (detailFromStore) {
      setNewNotice({
        id: detailFromStore.id,
        title: detailFromStore.title,
        content: detailFromStore.cont,
      });
      setAttachedFiles(
        (detailFromStore.files ?? []).map((file) => ({
          id: file.id,
          fileName: file.fileName,
          fileUrl: file.fileUrl,
          status: 'NORMAL',
