...
interface Props {
  initialData: NoticePage | null;
}

export default function NoticeClient({ initialData }: Props) {
  const router = useRouter();
  const fetchNotice = useFetchNoticeList();
  const noticeFromStore = useNoticeListStore((state) => state.notice);
  const noticeList = noticeFromStore ?? initialData;
