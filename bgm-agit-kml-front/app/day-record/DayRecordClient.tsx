...
import type { DayRecordResponse } from '@/services/server/dayRecord.server';

interface Props {
  initialData: DayRecordResponse | null;
}
...
export default function DayRecordClient({ initialData }: Props) {
  ...
  if (!hydratedRef.current && initialData) {
    setDayRecord(initialData);
    hydratedRef.current = true;
  }

  // 첫 마운트시 user에 따라 재페치 (권한별 응답 차이 반영) - SSR 데이터는 비로그인 기준
  const firstFetchSkipRef = useRef(true);
  useEffect(() => {
    if (firstFetchSkipRef.current && initialData && !user) {
      // SSR 데이터 그대로 사용
