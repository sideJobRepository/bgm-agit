...
public class InlineFileTracker {
    ...
    public void syncInlineFiles(String htmlContent, FileType type, Long targetId) {
        ...
        if (!currentKeys.isEmpty()) {
            List<BgmAgitFile> matched = bgmAgitFileRepository.findByPathsAndFileType(currentKeys, type);
            for (BgmAgitFile file : matched) {
                file.createTargetIdAndModifyCompleteFileStatus(targetId);
            }
        }

        // 2) 기존 targetId 에 묶여있던 인라인 중 본문에 없는 것 → TEMPORARY (배치 청소 대상)
        List<BgmAgitFile> existing = bgmAgitFileRepository.findByTargetIdAndFileType(targetId, type);
        for (BgmAgitFile file : existing) {
    ...
    public void releaseInlineFiles(FileType type, Long targetId) {
        List<BgmAgitFile> existing = bgmAgitFileRepository.findByTargetIdAndFileType(targetId, type);
        existing.forEach(BgmAgitFile::modifyTemporaryFileStatus);
    }
